...
export const executeFormula = async (req, res) => {
  try {
    ...
    const scaleMultiplier =
      scale === 'full' ? 1 : scale === 'half' ? 0.5 : 0.25;
    ...
    for (const ing of ingredientsToUse) {
      ...
      if (ing.unit !== product.baseUnit) {
        const subUnit = product.subUnits.find(
          (su) => su.name === ing.unit
        );
        if (subUnit) {
    ...
    const totalCost = ingredientsToUse.reduce((sum, ing) => {
      const product = formula.ingredients.find(
        (i) => i.product.toString() === ing.product.toString()
      );
      const price = ing.useBuyingPrice
    ...
    const productionData = {
      ...
      ingredients: ingredientsToUse.map((ing) => {
        const originalIng = formula.ingredients.find(
          (i) => i.product.toString() === ing.product.toString()
        );
        return {
          product: ing.product,
          ...
          quantity: ing.quantity,
          unit: ing.unit,
          unitCost: ing.useBuyingPrice
            ? originalIng?.buyingPrice || 0
            : originalIng?.sellingPrice || 0,
    ...
    if (formula.type === 'standard') {
      const finalOutputBags =
        outputBags !== undefined
          ? outputBags
          : (formula.defaultOutputBags || 0) * scaleMultiplier;

      const finalOutputKgs =
        outputKgs !== undefined
          ? outputKgs
          : (formula.defaultOutputKgs || 0) * scaleMultiplier;

      const outputQuantity =
        finalOutputBags + finalOutputKgs / 50;
      ...
      productionData.costPerUnit =
        outputQuantity > 0 ? totalCost / outputQuantity : 0;

      const finalProduct = await Product.findById(
        formula.finalProduct._id
      );
      ...
      if (sellImmediately && saleData) {
        const sale = await Sale.create({
          customer: { name: formula.customerName },
          items: [
            {
              product: null,
    ...
    if (productionData.sale) {
      response.sale = await Sale.findById(productionData.sale);
    }
